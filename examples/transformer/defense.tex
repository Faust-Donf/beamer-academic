...
% ============================================================
\documentclass[10pt, aspectratio=169, noamsthm]{beamer}
...
\begin{frame}[plain]
  \vskip1.2cm
  \begin{center}
    {\color{accentcolor}\rule{0.85\textwidth}{0.8pt}}\\[0.5cm]
    {\usebeamerfont{title}\usebeamercolor[fg]{title}Attention Is All You Need}\\[0.25cm]
    {\color{textgray}\fontsize{11pt}{14pt}\selectfont 基于纯注意力机制的序列转导模型}\\[0.3cm]
    {\color{accentcolor}\rule{0.85\textwidth}{0.8pt}}
  \end{center}
  \vskip0.8cm
  \begin{center}
    \begin{tabular}{r@{\;:\;}l}
      \textbf{Authors} & Ashish Vaswani, Noam Shazeer, Niki Parmar et al. \\[4pt]
      \textbf{Affiliation} & Google Brain / Google Research \\[4pt]
      \textbf{Venue} & Neural Information Processing Systems (NIPS) 2017 \\
    \end{tabular}
  \end{center}
  \vfill
  \begin{center}
    {\color{textgray}\scriptsize Paper Reading Presentation}
  \end{center}
  \vskip0.4cm
\end{frame}
}
...
% ============================================================
% P2: Table of Contents (tabbing format)
% ============================================================
\section*{目录}
\begin{frame}{目录}
\vskip0.4cm
{\large
\begin{tabbing}
\hspace{1.5cm}\=\hspace{0.4cm}\=\kill
\textcolor{accentcolor}{\bfseries 一} \> \textbf{研究背景与动机}\,——\,序列建模的瓶颈与注意力革命 \\[14pt]
\textcolor{accentcolor}{\bfseries 二} \> \textbf{模型架构}\,——\,Transformer 的核心设计 \\[14pt]
\textcolor{accentcolor}{\bfseries 三} \> \textbf{实验与结果}\,——\,机器翻译及其他任务验证 \\[14pt]
\textcolor{accentcolor}{\bfseries 四} \> \textbf{总结与展望}\,——\,贡献、局限与未来方向 \\
\end{tabbing}
}
\end{frame}
...
% P3: Outline page (section 1 highlighted)
\begin{frame}{报告提纲}
...
% P7: Outline page (section 2 highlighted)
\begin{frame}{报告提纲}
\tableofcontents[currentsection]
\end{frame}
...
\begin{tikzpicture}[
  ...
  \foreach \i/\c in {0/accentcolor!60, 1/accentcolor!40, 2/accentcolor!20} {
    \node[box, fill=\c!30] (lq\i) at (0, 1.2+\i*0.7) {\tiny Linear};
    \node[box, fill=\c!30] (lk\i) at (2.5, 1.2+\i*0.7) {\tiny Linear};
    \node[box, fill=\c!30] (lv\i) at (5.0, 1.2+\i*0.7) {\tiny Linear};
  }
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{层类型} & \textbf{每层复杂度} & \textbf{顺序操作数} & \textbf{最长路径} \\
\midrule
...
% P15: Outline page (section 3 highlighted)
\begin{frame}{报告提纲}
\tableofcontents[currentsection]
\end{frame}
...
\begin{tabular}{@{}llcc@{}}
\toprule
\textbf{模型} & \textbf{类型} & \textbf{EN-DE BLEU} & \textbf{EN-FR BLEU} \\
\midrule
ByteNet              & CNN         & 23.75 & ---  \\
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{变体} & $h$ & $d_k$ & $d_{\text{model}}$ & \textbf{BLEU} \\
\midrule
...
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{模型} & \textbf{WSJ 23 F1} & \textbf{训练数据} \\
\midrule
...
% P22: Outline page (section 4 highlighted)
\begin{frame}{报告提纲}
\tableofcontents[currentsection]
\end{frame}
...
本文的核心贡献可以凝练为以下三点，它们共同标志着序列建模范式的根本性转变：

\vskip0.3cm

\begin{enumerate}
\item \textbf{架构创新}\,——\,首次证明了\alert{纯注意力机制}足以完成序列转导任务，彻底消除了对循环和卷积结构的依赖，实现了 $O(1)$ 顺序操作的全并行训练
\item \textbf{性能突破}\,——\,在机器翻译基准上以显著优势超越所有已有方法，WMT EN-DE +2.0 BLEU、EN-FR 达到 41.8 BLEU 新纪录，且训练成本大幅降低
\item \textbf{范式影响}\,——\,Transformer 的成功开启了"预训练+微调"的大模型时代，BERT、GPT 系列、T5 等后续工作均以 Transformer 为基础架构，其影响远超机器翻译本身
\end{enumerate}
\end{frame}
...
% ============================================================
% P25: Thanks page (elegant text-only, no logo)
% ============================================================
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \fill[accentcolor] (current page.north west) rectangle
      ([yshift=-0.10cm]current page.north east);
    \fill[accentcolor] ([yshift=0.10cm]current page.south west) rectangle
      (current page.south east);
  \end{tikzpicture}
  \vbox to \paperheight\bgroup
    \vfill
    \begin{center}
      {\color{accentcolor}\sffamily\fontsize{11pt}{13pt}\selectfont\addfontfeature{LetterSpace=18.0}THANK YOU}
    \end{center}
    \vskip1.5cm
    \begin{center}
      {\color{accentcolor}\rule{2cm}{0.4pt}\hspace{0.8cm}{\small$\bullet$}\hspace{0.8cm}\rule{2cm}{0.4pt}}
    \end{center}
    \vskip1.2cm
    \begin{center}
      {\color{accentcolor}\sffamily\fontsize{30pt}{38pt}\selectfont\bfseries Thank You!}\\[0.5cm]
      {\color{accentcolor!70}\sffamily\fontsize{18pt}{22pt}\selectfont Questions?}
    \end{center}
    \vskip1.5cm
    \begin{center}
      {\color{textgray}\sffamily\fontsize{9pt}{11pt}\selectfont Vaswani et al.\;\;$\cdot$\;\;Google Brain / Google Research\;\;$\cdot$\;\;NIPS 2017}
    \end{center}
    \vfill
  \egroup
\end{frame}
